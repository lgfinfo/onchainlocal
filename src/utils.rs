...
use crate::kamino::{
    get_kamino_flashloan_borrow_ix, get_kamino_flashloan_repay_ix,
};
...
pub async fn calculate_dynamic_priority_fee(
    ...
    let mut prioritization_fees: Vec<u64> = fees
        .into_iter()
    ...
    let random_fees: Vec<u64> = (0..num_fees)
        ...
        .collect();
    info!("Returning {} random fees: {:?}", num_fees, random_fees);
    Ok(random_fees)
}
...
pub async fn extend_loan_start(instructions: &mut Vec<Instruction>, context: &Context) {
    ...
    let borrow_ix = get_kamino_flashloan_borrow_ix(
        ...
        get_associated_token_address(&context.payer_rc.pubkey(), &pubkey!("So11111111111111111111111111111111111111112")),
