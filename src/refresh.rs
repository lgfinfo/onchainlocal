...
pub async fn initialize_pool_data(
    ...
    info!("Pool data initialized for mint: {}", mint);
        for pool_address in pump_pools {
            let pump_pool_pubkey = Pubkey::from_str(pool_address)?;

            match rpc_client.get_account(pump_pool_pubkey).await {
                Ok(account) => {
        ...
        for pool_address in raydium_pools {
            ...
            match rpc_client.get_account(raydium_pool_pubkey).await {
                Ok(account) => {
        ...
        for pool_address in raydium_cp_pools {
            let raydium_cp_pool_pubkey = Pubkey::from_str(pool_address)?;

            match rpc_client.get_account(raydium_cp_pool_pubkey).await {
                Ok(account) => {
        ...
        for pool_address in dlmm_pools {
            let dlmm_pool_pubkey = Pubkey::from_str(pool_address)?;

            match rpc_client.get_account(dlmm_pool_pubkey).await {
                Ok(account) => {
        ...
        for pool_address in whirlpool_pools {
            let whirlpool_pool_pubkey = Pubkey::from_str(pool_address)?;

            match rpc_client.get_account(whirlpool_pool_pubkey).await {
                Ok(account) => {
        ...
        for pool_address in raydium_clmm_pools {
            let raydium_clmm_program_id = raydium_clmm_program_id();

            match rpc_client.get_account(Pubkey::from_str(pool_address)?).await {
                Ok(account) => {
